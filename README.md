...
The Nix flake pins the Python and `uv` toolchain; `requirements.lock` pins
direct and transitive Python package versions. The Docker image uses the same
flake and the fixed Nix base-image digest. On Linux, run:
